...
export default function Auth(){
  ...
  function selectHelperClickHandle(e: ChangeEvent<HTMLInputElement>){
    if (e.target.checked == true){
      if (helpers.filter(helper => helper.isDisabled == true).length < 3){
    ...
    <div id='auth'>
      ...
      <form onSubmit={submitHandler}
        ...
        <label>
          ...
          {sums.map(sum => <label><input type="radio"
                                         name="sum"
                                         value={sum}
                                         checked={sum == saveSum}
                                         onChange={e => setSaveSum(+e.target.value)} /> {sum}</label>)}


        </label>
        <p>Выберите подсказки (не более 3 штук)</p>
        <label>
          {helpers.map(helper => <label>
            <input type="checkbox"
                   value={helper.name}
